...
<img className="w-full h-auto" src="/images/interfaces/native-format/block-structure.webp" alt="Structure of a columnar wire block and its data layout families" width="1568" height="976" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/decoder-dispatch.webp" alt="Decoder routing from a type description to its encoding family" width="1568" height="854" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/array-offsets.webp" alt="Offsets mapping rows into a shared values stream" width="1568" height="388" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/nested-layout.webp" alt="Composite column layout with and without flattening" width="1172" height="1068" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/state-prefix-per-block.webp" alt="State prefix repeated for every non-empty block" width="1226" height="1162" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/variant-discriminators.webp" alt="Discriminators selecting values from dense type-specific runs" width="1568" height="624" loading="lazy" />
...
Typed paths are always materialized to their regular dense columns before `JSON` is written in the `Native` format, including when the source column is constant or compressed in memory, or when the `JSON` column is nested in `Array`, `Nullable`, `Tuple`, or a replicated representation. MergeTree parts may store an individual declared path with sparse serialization, but that per-path choice is not added to the outer column's `Native` kind stack and does not change the wire layout below. Sparse serializations elsewhere in an `Array` or `Nullable` subtree that contains `JSON` are materialized too, because those wrappers have no recursive serialization-info payload in the `Native` format.
...
<img className="w-full h-auto" src="/images/interfaces/native-format/compression-frame.webp" alt="Compression frame fields and compressed-size boundary" width="1568" height="206" loading="lazy" />
